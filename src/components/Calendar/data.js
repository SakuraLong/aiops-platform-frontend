...
class CalendarDataManager {
  ...
  async getDataSync() {
    // 先检索第一天的前一天的数据
    await this.queryDataByDayRange(this.SHOW_BEGIN - this.DAY, this.SHOW_BEGIN)
    if (this.type === 'day') return await this.queryDataByDayRange(this.SHOW_BEGIN, this.SHOW_BEGIN + this.DAY)
    else if (this.type === 'week') return await this.queryDataByDayRange(this.SHOW_BEGIN, this.SHOW_BEGIN + this.WEEK)
  }
   ...
   * @returns 这段时间内的数据
   */
  async queryDataByDayRange(begin = null, end = null) {
    if (!begin || !end) return []
    const res = []
    while (begin < end) {
